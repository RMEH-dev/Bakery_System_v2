...
const CheckoutModel = {
     ...
     getCartByUserId: async (userId, callback) => {
      ...
      createPayment: async (cartID, totalAmount, paymentType, callback) => {
        ...
        db.query(paymentQuery, [cartID, totalAmount, paymentType, paymentStatus], (err, result) => {
          if (err) {
            callback(err, null);
          } else {
      ...
      createOrder: async (userID, totalAmount, discountID, paymentID, orderType, deliveryID, addressID, paymentType, callback) => {
        const orderQuery = `
        ...
        db.query(orderQuery, [
          userID, totalAmount, discountID, paymentID, null, orderType, orderStatus, deliveryID, addressID
        ], (err, result) => {
          if (err) {
            callback(err, null);
          } else {
      ...
      createOrderDetails: async (orderID, cartItems, callback) => {
        ...
        try {
          for (const item of cartItems) {
            await db.query(orderDetailsQuery, [orderID, item.proStockBatchID, item.quantity]);
          }
          callback(null, true);
        } catch (err) {
          callback(err, null);
